...
interface AcademicRoomTableProps {
  data: RoomData[]
  purpose: string
...
export function AcademicRoomTable({ data, onEdit, onDelete, purpose }: AcademicRoomTableProps) {
  const [sorting, setSorting] = React.useState<SortingState>([])
  const [currentPage, setCurrentPage] = React.useState(1)
  const pageSize = 5

  console.log("RoomDataTable data:", data)

  const columns: ColumnDef<RoomData>[] = [
    {
      accessorKey: "room_name",
      header: "Room Name",
      cell: ({ row }) => <div className="capitalize">{row.getValue("room_name")}</div>,
    },
    { accessorKey: "room_number", header: "Room Number" },
    { accessorKey: "seating_capacity", header: "Room Capacity" },
    {
      accessorKey: "actions",
      header: "Actions",
      cell: ({ row }) => {
