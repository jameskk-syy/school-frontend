...
import { StudentCategory, StudentCategoryTable } from "./student_category_table"
import { format } from "date-fns"

export function TabsStudentCategory() {
  const [categoryDatas, setcategoryDatas] = useState<StudentCategory[]>([])        // Add tab data
  const [categoyData, setcategoyData] = useState<StudentCategory[]>([])          // View tab data
  ...
  const addOrUpdatecategoryDatas = (e: any) => {
    ...
    setFormData({ category: "", name: "", creation: "", modified: "" })
  }
  ...
  const addOrUpdatecategoyData = (e: any) => {
    ...
    setFormDatas({ category: "", name: "", creation: "", modified: "" })
  }
  ...
  const savecategoryDatas = (e: any) => {
    ...
    setIsLoading(true)
    api.post("school_app.services.rest.create_student_category", JSON.stringify(categoryDatas))
      .then((res) => {
        setIsLoading(false)
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <Tabs defaultValue="add">
        <TabsList className="md:mb-2 lg:mb-2">
          <TabsTrigger value="add">Add New Student Category</TabsTrigger>
          <TabsTrigger value="view">View All Student Categorys</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="add" className="w-full">
          <form onSubmit={addOrUpdatecategoryDatas}>
            <Card>
              <CardHeader>
                <CardTitle>{editingIndexAdd !== null ? "Edit Student Category" : "Create Student Category"}</CardTitle>
                <CardDescription>Enter data below and click {editingIndexAdd !== null ? "Update" : "Add"}.</CardDescription>
              </CardHeader>

              <CardContent className="grid md:grid-cols-2 gap-6">
                <div className="grid gap-3">
                  <Label htmlFor="category">Student Category Name<span className="text-red-800"> *</span></Label>
                  <Input
                    name="category"
              ...
              <CardFooter>
                {isLoading ? <Spinner /> : <Button type="submit">{editingIndexAdd !== null ? "Student Category" : "Student Category"}</Button>}
              </CardFooter>
            </Card>
          </form>
          <StudentCategoryTable data={categoryDatas} onEdit={handleEdit} onDelete={handleDelete} purpose="add" />
          {isLoading ? <Spinner /> : <Button onClick={savecategoryDatas} className="md:mt-4 sm:mt-2">Save Student Category</Button>}
        </TabsContent>
        ...
        <TabsContent value="view">
          <Card>
            <CardHeader>
              <CardTitle>Here Are All Student Category</CardTitle>
              <CardDescription>This is a list of Student Category</CardDescription>
            </CardHeader>
            ...
            <CardContent className="grid md:grid-cols-1 gap-6">
              <form onSubmit={addOrUpdatecategoyData}>
                <Card>
                  <CardHeader>
                    <CardTitle>{editingIndexView !== null ? "Edit Student Category" : "Add Student Category"}</CardTitle>
                  ...
                  <CardFooter>
                    {isLoading ? <Spinner /> : <Button type="submit" disabled={editingIndexView == null}>Update Student Category</Button>}
                  </CardFooter>
                </Card>
              </form>
              <StudentCategoryTable data={categoyData} onEdit={handleEdits} onDelete={handleDelets} purpose="view" />
            </CardContent>

            <CardFooter>
              {isLoading ? <Spinner /> : <Button onClick={updatecategoryDatas} >Submit Student Category</Button>}
            </CardFooter>
          </Card>
        </TabsContent>
      </Tabs>
