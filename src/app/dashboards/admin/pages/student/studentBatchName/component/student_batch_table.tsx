...
interface StudentBatchNameTableProps {
  data: StudentBatchName[]
  purpose: string
  onEdit: (index: number) => void
  onDelete: (index: number) => void
}

export function StudentBatchNameTable({ data, onEdit, onDelete, purpose }: StudentBatchNameTableProps) {
  const [sorting, setSorting] = React.useState<SortingState>([])
  const [currentPage, setCurrentPage] = React.useState(1)
  const pageSize = 5

  console.log("StudentBatchNameTable data:", data)

  const columns: ColumnDef<StudentBatchName>[] = [
    {
      accessorKey: "name",
      header: "ID",
    ...
    { accessorKey: "creation", header: "Created At" },
     { accessorKey: "modified", header: "Modified At" },
    {
      accessorKey: "actions",
      header: "Actions",
      cell: ({ row }) => {
